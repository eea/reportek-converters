    ...
    if converter:
        ...
        try:
            ...
            try:
                ...
                conversion_log.warning(
                    message % (converter_id, "[unable to display]\n")
                )
            else:
...
def list_converters_params():
    ...
    for conv in list(converters.values()):
        name = "{prefix}{name}".format(
            prefix=app.config.get("PREFIX", ""), name=conv.name
        )
        tag = app.config.get("TAG", "")
        if tag:
            title = "{title} {tag}".format(title=conv.title, tag="(%s)" % tag)
        else:
...
class Converter(object):
    def __init__(
        ...
        self.suffix = suffix
        self.__compatibility__init__(title, ct_schema, description)
