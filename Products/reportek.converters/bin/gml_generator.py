...
class GMLGenerator:
    ...
    def fillMetadata(self, val, filename):
        ...
        fmp_a(
            METADATA_TEMPLATE
            % (
                utGMLEncode(val["met:organisationName"], self.__user_enc),
    ...
    def fillExportMetadata(self, val, filename):
        ...
        fmp_a(
            '<met:info xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"'
        )
        fmp_a(
            'xmlns:met="http://biodiversity.eionet.europa.eu/schemas/dir9243eec"'
        )
        fmp_a(
            'xsi:schemaLocation="http://biodiversity.eionet.europa.eu/schemas/dir9243eec http://biodiversity.eionet.europa.eu/schemas/dir9243eec/gml_art17_meta.xsd" href="%s.gml">'
            % (filename)
        )
        fmp_a(
            METADATA_TEMPLATE
    ...
    def fillFeatureMemberPoint(self, fID, X, Y, dbfValues):
        ...
        for k, v in list(dbfValues.items()):
            fmp_a(
                "<%s>%s</%s>"
                % (
                    utGMLEncode(k, self.__user_enc),
                    utGMLEncode(v, self.__user_enc),
                    utGMLEncode(k, self.__user_enc),
                )
            )
        fmp_a("</reportnet>")
    ...
    def fillFeatureMemberLine(self, fID, vertices, dbfValues):
        ...
        if len(vertices) == 1:
            ...
            for k in range(len(vertices)):
                ...
                fml_a(
                    "<gml:coordinates>%s</gml:coordinates>" % strip(l_vmtoadd)
                )
                fml_a("</gml:LineString>")
                fml_a("</gml:lineStringMember>")
            fml_a("</gml:MultiLineString>")
    ...
    def fillSDHeader(self):
        ...
        h_a(
            '<xs:import namespace="http://www.opengis.net/gml" schemaLocation="http://schemas.opengis.net/gml/3.1.1/base/geometryBasic0d1d.xsd"/>'
        )

        h_a(
            '<xs:element name="FeatureCollection" type="gml:FeatureCollectionType" substitutionGroup="gml:_FeatureCollection"/>'
        )
        ...
        h_a(
            '<xs:element name="metaDataProperty" substitutionGroup="gml:_MetaData">'
        )
        h_a("<xs:complexType>")
        ...
        h_a("<xs:sequence>")
        h_a(
            '<xs:element name="datasetName" minOccurs="0" nillable="true" type="xs:string"/>'
        )
        h_a("</xs:sequence>")
        h_a("</xs:extension>")
        h_a("</xs:complexContent>")
