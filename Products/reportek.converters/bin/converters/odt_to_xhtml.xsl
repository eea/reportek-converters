...
<xsl:template match="style:style">
	<xsl:choose>
		...
		<xsl:when test="@style:family='table-row'">
			<xsl:call-template name="process-table-row-style"/>
		</xsl:when>
...
<xsl:template name="process-table-row-style">
	<xsl:if test="style:table-row-properties">
		<xsl:value-of select="$lineBreak"/>
		<xsl:text>.</xsl:text>
		<xsl:value-of select="translate(@style:name,'.','_')"/>
		<xsl:text>{</xsl:text>
		<xsl:if test="style:table-row-properties/@style:row-height">
			<xsl:text>height: </xsl:text>
			<xsl:value-of select="style:table-row-properties/
				@style:row-height"/><xsl:text>;</xsl:text>
			<xsl:value-of select="$lineBreak"/>
		</xsl:if>
		<xsl:call-template name="handle-style-properties">
			<xsl:with-param name="nodeSet"
...
<xsl:template name="process-style">
	<xsl:if test="style:table-cell-properties or style:paragraph-properties or style:text-properties">
		<xsl:value-of select="$lineBreak"/>
		<xsl:text>.</xsl:text>
		<xsl:value-of select="translate(@style:name,'.','_')"/>
	...
	<xsl:for-each select="$nodeSet/@*">
		...
		<xsl:if test="$find">
			...
			<xsl:choose>
				...
				<xsl:when test="$action='font-name'">
					<xsl:call-template name="font-name"/>
				</xsl:when>
			</xsl:choose>
		</xsl:if>
	</xsl:for-each>
...
<xsl:template name="font-name">
	<xsl:text>font-family: "</xsl:text>
	<xsl:value-of select="."/><xsl:text>", sans-serif;</xsl:text>
	<xsl:value-of select="$lineBreak"/>
</xsl:template>

<xsl:template match="text:table-of-content-source|text:illustration-index-source"/>
...
<int:attr-map>
	...
	<int:attr name="fo:font-size" action="pass-through"/>
	...
	<int:attr name="style:font-name" action="font-name"/>
</int:attr-map>
</xsl:stylesheet>
